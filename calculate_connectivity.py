...
for sub_ID in sub_IDs:
    ...
    if cfg.n_cycles[0]=='fixed':
        n_cycles = cfg.n_cycles[1]
    elif cfg.n_cycles[0]=='diff':
        n_cycles = freqs / cfg.n_cycles[1]
    ...
    if cfg.rois_as_seed:
        for roi in rois:
            ...
            roi_label = '%s-%s' % (roi.name[0:2], roi.name[-2:])
            # output filename
            ...
            for cond in conds:
                ...
                cons, freqs, _, n_epochs, _ = mne.connectivity.spectral_connectivity(
                    tcs, method=cfg.con_method, mode=cfg.con_mode, indices=inds, 
                    sfreq=sfreq, cwt_freqs=freqs, cwt_n_cycles=n_cycles, n_jobs=cfg.n_jobs)
                ...
                if cfg.average_times:
                    if cond=='Baseline':
                        ...
                        i_tmax = i_tmax_bl
                    if type(cons)==list:
                        cons = [con[:,:,i_tmin:i_tmax].mean(-1) for con in cons]
                    else:
                        cons = cons[:,:,i_tmin:i_tmax].mean(-1)
            
                if cfg.average_freqs:
                    if type(cons)==list:
                        cons = [con.mean(1) for con in cons]
                    else:
                        cons = cons.mean(1)
                
                cons_dict.update({cond: cons})
